...
something real. Finished stops stay on the page, so the whole walk can be read
back, and the letters some stops leave behind are put together at the end.
...
## Layout
    ...
    pipeline/plaques.py    draw the two plaque stencils (needs Pillow; not the build)
...
The whole format:
    ...
    finale: SMYTH EDINBURGH      optional; what every stop's letters spell
    ...
    reveal: Jewel Tower          the name the card takes when the stop is finished
    ...
    letters: MYBURGH, scrambled  what the stop leaves on its card
    ...
    ## Outro                     the ending, after the letters
...
- A **picture** is an ordinary Markdown image line, alone, with its caption as
  the quoted title: `![what it shows](img/thing.png "The caption")`. It is
  uncovered from the top down when its turn comes, and the words after it
  wait for it.
...
and `title <words>`, and with none of those the name comes with the finish
(see below); a `gate` takes `compass`, `compass throughout` and `distance`
after the radius; a `lens` takes `auto`, `match <n>`, `hold <n> ms`, `gated`,
`not gated`, `fallback <n> s` and `no fallback` after the picture; `letters`
takes `scrambled` or `in order`. Commas separate those, so a chapter or a
stencil note that needs a comma of its own is fine, but an answer is not.
...
on the phone. The previews are documentation and are not shipped to the player.
...
**The plaque stencils** are different: a frame, four bolt heads, and only the
letters the hunt wants noticed, each drawn solid exactly where it sits on the
plaque, at 1600 px wide because two-centimetre letters on a metre-wide plaque
vanish at 800. `pipeline/plaques.py` draws them from measurements of a
perspective-corrected photograph of each plaque, and which letters show is a
few characters in that file. It needs Pillow, which the build does not.

![The Imperial Standards of Length stencil over its plaque](app/img/imperial-standards-stencil-preview.jpg)

![The Standard Chain stencil over its plaque](app/img/standard-chain-stencil-preview.jpg)

## The walk on the page

- A **finished stop stays on the page** above the open one, whole: its words,
  its pictures, the answer that was typed, and the letters it left. Every
  control goes with the finish: the location check, the camera button, the
  question's box, Skip, Next. A coloured edge and a word in the header say how
  it went. The page scrolls; the open stop is scrolled to.
- **Letters.** A stop with `letters:` leaves them on its card as tiles once it
  is finished, however it was finished. `scrambled` shows them in a fixed
  shuffle, the same on every phone and never the true order; `in order`, the
  default, shows them as written.
- **The ending.** The last stop's button is **Finish**. If the hunt has a
  `finale`, the ending first shows every stop's letters on one line, in the
  stops' order and the true order within each stop, arriving one at a time;
  then, after a held moment, each letter slides to its place and the line
  reads the finale. The build refuses a finale the letters cannot spell. Then
  the outro, and in test mode the tally; with test mode off, the outro is the
  last thing on the page.
- **Finishing a reveal** is three taps within a second and a half on the text
  that is arriving. One or two taps do nothing. It finishes a picture that is
  uncovering as well as a paragraph.
...
which are Begin, Continue, Open the camera, Progress, Next, Finish, and a
right answer. Those buttons carry `data-strong`; that is the one place to tune.
...
- **`test_mode: yes`** at the top of a hunt turns on every player-facing
  ...
  match and every question; the threshold slider in the lens; a Start over
  button in the bar at the top of the walk that is always on screen; and the
  tally at the end. Skipped parts count in the tally as skipped, not wrong.
  One line: make it `no` and rebuild before anybody plays for real, and none
  of them exist. Start over stays in the menu either way. `#testing` in the
  address is separate: that is the position simulator, a developer tool.
...
- **`compass`** on a gate line shows one of eight points while the phone is
  more than five hundred metres out on a usable fix, and nothing at all
  inside that. **`compass throughout`** keeps the point all the way to the
  door. **`distance`** shows the distance to the target from each usable fix
  while the gate card is open, rounded the way a person would say it.
- **`reveal: Jewel Tower`** on a stop keeps the stop's written chapter and
  title until the stop is finished, then changes them, once, for good.
  Finished means Progress on its match if it has one, else its question
  answered, right or out of guesses, else its gate passed. A skip finishes
  nothing, so it names nothing. **`reveal: Westminster, at 300 m`** on a stop
  with a gate changes them instead when a usable fix reads within that
  distance, or on the pass; `on the pass` means only on the pass. At least
  one of the new chapter and the new title (`title <words>`) is required.
